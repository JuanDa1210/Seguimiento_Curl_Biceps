...
if brazo_elegido == "Izquierdo":
    ...
    nombre_archivo_csv = "Prf_izq.csv" # Nombre de archivo consistente
    ...
    nombre_archivo_csv = "Prf_der.csv" # Nombre de archivo consistente
...
with mp_pose.Pose(static_image_mode=False, min_detection_confidence=0.7, min_tracking_confidence=0.7) as pose:
    ...
    while True:  # Quitamos la condición del tiempo, controlamos manualmente
        ret, frame = cap.read()
        if not ret:
        ...
        if results.pose_landmarks:
            ...
            if start_time is None:
                cv2.putText(image_bgr, "Esperando detección...", (30, 50),
                            cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2, cv2.LINE_AA)

        cv2.imshow('Detección de Ángulo', image_bgr)
